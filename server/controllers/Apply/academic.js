...
const createApply = async (req, res) => {
  const { fullname, phone, email, courseID } = req.body;
  

  try {
    const existingApplication = await AcademicApply.findOne({
      where: { email }
    });
    ...
    const newData = await AcademicApply.create({
      fullname,
      phone,
      email,
      courseID
...
const getApplies = async (req, res) => {
  try {
    const allData = await AcademicApply.findAll(
      {
        include: {
          model: Course,
          as: 'course',
...
const deleteApply = async (req, res) => {
  ...
  try {
    const deletedRows = await AcademicApply.destroy({
      where: { id }
    });
